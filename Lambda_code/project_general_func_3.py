...
def lambda_handler(event, context):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            # Query to get detailed gender employment data from employmentratio_details table
            query = """
            SELECT 
                primary_division_name, 
                submission_group_size,
                SUM(Men) as Men, 
                SUM(Women) as Women, 
                SUM(difference) as difference, 
                SUM(total_employees) as total_employees,
                AVG(gap_ratio) as gap_ratio
            FROM employmentratio_details
            GROUP BY primary_division_name, submission_group_size
            """
            ...
            processed_data = [
                {
                    ...
                    "submission_group_size": row['submission_group_size'],
