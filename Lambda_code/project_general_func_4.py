...
def lambda_handler(event, context):
    ...
    try:
        ...
        company_name = event.get('queryStringParameters', {}).get('company_name', '')

        if not company_name:
            raise ValueError("Company name must be provided in the query parameters")

        logger.info(f"Querying for company: {company_name}")
        ...
        with connection.cursor() as cursor:
            query = """
            SELECT *
            FROM workforce_composition
            WHERE TRIM(primary_employer_name) = TRIM(%s)
            """
            ...
            if not result:
                ...
                return {
                    'statusCode': 404,
                    'body': json.dumps({"error": f"No data found for company: {company_name}"})
                }
