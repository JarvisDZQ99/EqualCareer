...
def lambda_handler(event, context):
    ...
    try:
        with connection.cursor() as cursor:
            # Check if 'industry' and 'year' parameters are provided
            params = event.get('queryStringParameters', {})
            industry = params.get('industry')
            year = params.get('year')
            
            if industry and year:
                query = "SELECT * FROM paygap_industry WHERE industry = %s AND y_m LIKE %s"
                cursor.execute(query, (industry, f"{year}%"))
            elif industry:
                query = "SELECT * FROM paygap_industry WHERE industry = %s"
                cursor.execute(query, (industry,))
            elif year:
                query = "SELECT * FROM paygap_industry WHERE y_m LIKE %s"
                cursor.execute(query, (f"{year}%",))
            else:
                query = "SELECT * FROM paygap_industry"
