...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not industry:
            raise ValueError("Industry must be provided in the query parameters")

        logger.info(f"Querying for industry: {industry}")
        ...
        with connection.cursor() as cursor:
            # Query to find occupations by industry
            query = """
            SELECT industry, occupation
            FROM industry_occupation
            ...
            if not result:
                ...
                return {
                    'statusCode': 404,
                    'body': json.dumps({"error": f"No data found for industry: {industry}"})
                }
